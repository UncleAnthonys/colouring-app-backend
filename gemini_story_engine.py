...
* **Episode 1 Goal Contract (ALL AGES — NON-NEGOTIABLE):** Episode 1 must establish FIVE things in plain, age-appropriate language:
  ...
  5. DOES THE SOLUTION MATCH THE PROBLEM? (The SOLUTION TYPE must match the PROBLEM TYPE. If the problem is physical — breaking, falling, flooding, crumbling — the fix must be physical: stick it, push it, block it, build it. If the problem is a creature — angry, scared, lost — the fix must involve the creature: calm it, feed it, guide it. NEVER solve a physical problem with music, feelings, magic sounds, or unrelated actions. TEST: Could a child explain WHY the solution works? "She stuck the cracks together so it stopped breaking" = YES, a child gets that. "She played music and it stopped breaking" = NO — a child would ask "but WHY does music fix cracks?" If the child would ask "but why?", the solution doesn't match the problem. GOOD: Moon is crumbling → character stuffs the cracks with sticky moon-cheese to hold it together. BAD: Moon is crumbling → character plays a music box and it magically stops. GOOD: Robot is overheating → character dumps cold water on it. BAD: Robot is overheating → character sings a song and it cools down.)
  ...
  Without these five things, the story has no engine. A reader must finish episode 1 feeling URGENCY — not just knowing what the character wants, but feeling the ticking clock, fearing the consequence, and understanding HOW the mission saves the day.

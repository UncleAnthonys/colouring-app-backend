...
(e2) PITCH SHAPE AND SETTING ROTATION RULE (under_3 AND age_3):
   ...
   FOUR OTHER VALID SHAPES (each is a real picture-book template — pick whichever shape best fits the brainstorm you start with):
     ...
     - WAITING-AND-RETURN (Owl Babies by Martin Waddell shape): hero waits for someone or something to come back. Each page they wonder, watch, or worry. Final page = arrival. e.g. "Mabel is waiting for Mum to come home. Maybe in the kitchen? Maybe in the hall? Maybe at the door? Mum is back!" Reach for this shape when the brainstorm involves anticipation — waiting for a parent, a pet, the post, a friend, the sun, the rain to stop.
     - ROUTINE-COMPLETION (Goodnight Moon by Margaret Wise Brown shape): hero says goodnight to / finishes with / packs up a series of objects. Final page = routine done. e.g. "Mabel says goodnight to the boots. Goodnight to the bath. Goodnight to the toys. Goodnight to the moon. Mabel is asleep." Reach for this shape when the brainstorm involves a familiar toddler routine — bedtime, bathtime, mealtime, getting dressed, tidying up, going on a journey.
     - TRANSFORMATION (Hungry Caterpillar by Eric Carle shape): the problem-state changes naturally over the pages. e.g. "Caterpillar eats one apple. Two pears. Three plums. Now caterpillar is a butterfly!" Reach for this shape when the brainstorm involves something becoming something else — food disappearing as it's eaten, ice melting, wet things drying, small things growing.

   BALANCE: across multiple pitch generations for the same character, aim for genuine variety. CHASE-AND-CATCH is the most common shape and the safe default — but if EVERY pitch comes back as a chase, the user's child reads near-identical stories on rotation. When the brainstorm naturally points at one of the four other shapes, USE that shape. Don't force a chase onto a brainstorm that isn't really one. The four alternatives are equally valid; pick the one that actually fits the situation you've imagined.

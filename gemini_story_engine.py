...
(e2) PITCH SHAPE AND SETTING ROTATION RULE (under_3 specific):
   ...
   OPTIONAL ALTERNATIVE SHAPES (only use if the pitch GENUINELY fits — these are harder to execute well at 15 words/page, so a solid chase story beats a broken alternative):
     - REPEATED-ARRIVAL (Dear Zoo by Rod Campbell shape): something keeps arriving, each one wrong in a different way, final one is just right. e.g. "The post box keeps delivering the wrong puppy for Mabel — too big! too small! too wiggly! — until just the right one arrives." NO chase. NO tool. Use ONLY if the pitch is naturally about repeated reveals.
     - WAITING-AND-RETURN (Owl Babies by Martin Waddell shape): hero waits for someone or something to come back. Each page they wonder, watch, or worry. Final page = arrival. e.g. "Mabel is waiting for Mum to come home. Maybe in the kitchen? Maybe in the hall? Maybe at the door? Mum is back!" Use ONLY if the pitch is naturally about waiting for someone/something specific to return.
     - ROUTINE-COMPLETION (Goodnight Moon by Margaret Wise Brown shape): hero says goodnight to / finishes with / packs up a series of objects. Final page = routine done. e.g. "Mabel says goodnight to the boots. Goodnight to the bath. Goodnight to the toys. Goodnight to the moon. Mabel is asleep." Use ONLY for genuine bedtime / tidying / pack-up routines.
     - TRANSFORMATION (Hungry Caterpillar by Eric Carle shape): the problem-state changes naturally over the pages. e.g. "Caterpillar eats one apple. Two pears. Three plums. Now caterpillar is a butterfly!" Use ONLY if the pitch is naturally about something becoming something else (food disappearing as it's eaten, ice melting, wet thing drying).

   The four alternatives are real shapes from real picture books. They're available when the pitch fits one of them. They are NOT a quota — do not produce one just to vary shape. Most pitches will be CHASE-AND-CATCH and that is correct.

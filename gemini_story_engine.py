...
* **Cozy Landing (age-dependent):** For under_3 and age_3 ONLY — Episode 5 must return the character to safety and warmth. A "big hug" or "everything's OK" ending. For age_4 and above — Episode 5 can end on a joke, a surprising mess, or a funny twist. It does NOT need to be cosy. It needs to be SATISFYING — the physical problem is resolved in a way that makes the parent laugh. CRITICAL FOR ALL AGES: The final page must end on a physical moment or a single warm image — the emotion must be VISIBLE in the action, not stated. NEVER end with a summarising sentence about how everyone feels ("Everyone feels safe and happy", "They were all so pleased", "It had been a wonderful day"). These are lazy AI endings. The action IS the ending. GOOD: "SNUGGLE! Toastie rests on his warm plate. He is a very helpful snack." GOOD: "Jane catches the lucky coin. She gives Incy a big hug. YAY!" BAD: "Everyone felt happy and safe now. It had been quite an adventure." If you have written a summarising final sentence, delete it. THE LAST WORD RULE (under_3 and age_3 ONLY): The very last word of the entire story must be a sound word (YAY, YUM, SNAP, POP, BUZZ) or a direct physical action (HUG, SMILE, SLEEP). This gives the parent a natural curtain call. For age_4 and above, the ending must still land on a physical moment or image — but it can be a full sentence with personality rather than a single word.
* **Age Calibration (STRICT — match the tier):**
  - **TODDLER PULSE (under_3 ONLY):** Pure board book. Episode 1 MUST establish the setting and goal in plain simple words — even within 10 words. GOOD: "Al is in the library. SHHH!" (setting + goal + sound). GOOD: "Tim wants the big red ball. BOUNCE!" Every page must make sense on its own — a child must understand WHERE they are and WHAT is happening. Sound-Action-Sound structure — lead every page with a sound word, then the action, then a second sound that is the DIRECT RESULT of that action. "WOBBLE! Hannah tips the box. CRASH!" MAXIMUM 10 WORDS TOTAL PER PAGE — count them. If you go over, cut until a parent can read it in a single breath. Single object focus. Stick to first-100-words vocabulary.
    **SOUND WORD RULES (CRITICAL — this is where under_3 stories fail):**
    (a) A sound word is ONOMATOPOEIA — a word that imitates a NOISE you can physically hear. SPLASH, CRASH, POP, SNAP, DRIP, THUD, CRUNCH, SWOOSH, PLOP, BANG, RUSTLE, SQUELCH, CREAK, WHOOSH are sound words. TRIP, HUG, BOUNCE, CATCH, PEEK, SMILE are NOT sound words — they are verbs. Test: close your eyes and ask "could I HEAR this noise in real life?" If not, it is banned.
    (b) The sound must match the MATERIAL and PHYSICS of the action. A leaf collision makes RUSTLE or SWOOSH — never BUMP. A grape rolling makes a low RUMBLE or WOBBLE — never BOUNCE. A twig breaking makes SNAP — never TRIP. Ask: "what would this object ACTUALLY sound like?" and use THAT word.
    (c) Both sound words on each page must describe the SAME physical moment from two angles. GOOD: "SPLASH! Teddy fell in the puddle. SPLOSH!" (both = water). BAD: "BOUNCE! The grape rolls away. WHOOSH!" (two unrelated events). The closing sound must be a CONSEQUENCE of the action sentence, not a random exclamation.
    (d) After writing each page, re-read the action sentence and ask: "Does the opening sound CAUSE or DESCRIBE this action? Does the closing sound RESULT from this action?" If either answer is no, rewrite the sounds.
    **REPEATING ANCHOR (CRITICAL — without this, under_3 feels random):** Every under_3 story MUST have ONE repeating element on pages 2, 3, and 4. Choose one: (1) A refrain phrase the child anticipates — e.g. "Come back, grape!" or "Where did it go?" appears after each failed attempt. (2) A recurring sound word — e.g. ROLL! anchors every page because the grape keeps rolling. Page 1 sets up the goal. Pages 2-4 use the repeating anchor. Page 5 BREAKS the pattern — different sound, different outcome, problem solved. The repetition is what makes a board book a board book. Without it the story is just 5 disconnected captions.
  - **TODDLER NARRATIVE (age_3 ONLY):** The "Bridge Tier" between board book and preschool. 15-25 words per page. 2-3 short sentences. Use Subject-Verb-Descriptor structure ("Milo hits the big drum. It makes a loud BANG!"). CRITICAL: the verb must be physically expressive and simple — use "hunts" not "looks", "swishes" not "shakes", "tumbles" not "falls", "scrambles" not "moves". Every verb should feel like movement when read aloud. Never use flat, passive verbs like "sees", "goes", "is", "gets". Occasionally (not every page) add a short question or exclamation after the action to pull the child in — "Where did it go?" or "Oh no!" — but only when it feels natural, never forced. ATTENTION GRABBERS: Use tiny direct address words (LOOK!, Listen!, Oh no!) to direct the child's eye to the specific thing happening on the page. GOOD: "The big apple wobbles. LOOK! It tumbles off the branch." GOOD: "A tiny bug crawls past. Listen! It makes a tickly sound." Use sparingly — one per page maximum, only when it genuinely adds energy. Each page is a Micro-Mission with Causal Sequence logic: Setup → Action → Result. Use prepositional action words (under, over, through, behind) as active voice anchors. Include EXACTLY ONE sound word per page in ALL CAPS — not two, not zero. The sound word must directly describe the result of the action on that page. It cannot float after an unrelated sentence. WRONG: "Jamie sees the apple. SWISH!" RIGHT: "The apple drops from the tree. PLOP!" Use "Join-In" repetition — a phrase that repeats every 2 pages so the child can predict and shout it. CRITICAL: the join-in phrase must have a TRIGGER CONDITION — it only appears when a specific thing goes wrong (the object falls, slips, bounces away, escapes). It must NEVER appear after a successful action or a neutral observation. WRONG: "The jelly sits safely on the table. YUM! Oh no, the jelly!" (nothing went wrong — no trigger). RIGHT: "SPLAT! The jelly slipped again. Oh no, the jelly!" (the phrase fires because the thing went wrong). ADDITIONAL RULE: the refrain must NEVER appear on the resolution page (the page where the goal is achieved or the problem is solved). If the character just succeeded, the refrain is banned — use a warm celebratory line instead. A character's feature causing a side effect during a SUCCESS is NOT a trigger — the refrain only fires during genuine failures or setbacks. The story has a simple goal stated on page 1 and simple attempts (too high! too slippery!) leading to success.

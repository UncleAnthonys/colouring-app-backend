...
The join-in phrase must be tied to the CHARACTER'S GOAL or EMOTIONAL STATE — not a description of what an object is doing. It must still make sense on every single page it appears because it captures WHY they are trying, not WHAT is currently happening. GOOD: "We have to find those clues!" or "Come on, we can do it!" BAD: "The map is slipping!" (situational — stops making sense the moment the situation changes). The phrase should be warm, funny, or celebratory. It must only appear when a specific thing goes wrong — never after a calm or successful moment. CRITICAL: The refrain must be completely self-contained — a child must understand it with zero context from the surrounding sentences. Never use pronouns like "them", "it", "they", "her" unless the noun they refer to is inside the refrain itself. WRONG: "Oh no, wipe them dry!" (what is "them"? Never established). WRONG: "Oh no, it slipped!" (what is "it"?). RIGHT: "We must find that key!" (self-contained — no ambiguous pronouns). RIGHT: "Oh no, the rabbit is gone!" (names the subject explicitly).
